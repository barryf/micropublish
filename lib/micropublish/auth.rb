module Micropublish
  module Auth
    ...
    def callback(me, code, state, redirect_uri)
      ...
      return unless confirm_auth?(me, code, state, redirect_uri, endpoints[:authorization_endpoint])

      # find out if we're allowed a token to post
      token = get_token(me, code, state, redirect_uri, endpoints[:token_endpoint])
      return if token.nil?
    ...
    def find_endpoints(me)
      ...
      if response.code == 200
        ...
        doc.css('link').each do |link|
          ...
          elsif link[:rel].downcase == 'authorization_endpoint' && !link[:href].empty?
